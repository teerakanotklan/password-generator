...
- **Layout Model**: Responsive 2-column desktop partition (max-height 690px with internal Base UI `ScrollArea` for passwords), borderless password display panel on the left, card configuration controls on the right.
...
- `components/generate-password-form.tsx`: Primary orchestrator combining form state, debounced generation, copy/download actions, error display, and column layouts.
- `components/password-display-list.tsx`: Scrollable list rendering passwords with per-character syntax highlighting (numbers in blue, symbols in bold amber), click-to-copy, and status feedback.
...
- `components/password-length-control.tsx`: Slider and number input (4–64 characters).
- `components/password-quantity-control.tsx`: Slider and number input for batch generation (1–500 passwords).
...
- `components/password-generator-skeleton.tsx`: Borderless hydration skeleton matching exact layout to prevent layout shift.
